class IntentDrivenShortcutsService {
  constructor() {
    this.shortcuts = {
      ...
      'generate pre-call brief': this.generatePreCallBrief.bind(this),
    };
  }
  ...
  async generatePreCallBrief(data) {
    const { prospectId } = data;
    const voiceAgentIntegration = new VoiceAgentIntegration(process.env.API_KEY, process.env.API_URL);
    const preCallBriefDashboard = await voiceAgentIntegration.generatePreCallBrief(prospectId);
    return preCallBriefDashboard.render();
  }
}
